...
export class HighlightRenderer {
   ...
   */
  private groupOverlappingHighlights(segments: HighlightSegment[]): HighlightSegment[][] {
  ...
  private createHighlightElement(
    ...
    if (segments.length > 0) {
      const tooltipText = segments
        .map(s => `${s.userName || 'Anonymous'}${s.note ? ': ' + s.note : ''}`)
        .join('\n')
  ...
  public getHighlightAtPosition(x: number, y: number): Id<'highlights'> | null {
    ...
    return highlightElement.getAttribute('data-highlight-id') as Id<'highlights'>
  }

...
export const WALLET_METADATA: Record<string, WalletInfo> = {
  ...
  [XBULL_ID]: { id: XBULL_ID, name: 'xBull', type: 'xbull', requiresEagerConnection: true },
  [ALBEDO_ID]: { id: ALBEDO_ID, name: 'Albedo', type: 'albedo', requiresEagerConnection: true },
  [RABET_ID]: { id: RABET_ID, name: 'Rabet', type: 'rabet' },
  [HANA_ID]: { id: HANA_ID, name: 'Hana', type: 'hana', requiresEagerConnection: true },
  [HOTWALLET_ID]: { id: HOTWALLET_ID, name: 'HOT Wallet', type: 'hot' as WalletType, requiresEagerConnection: true },
};
...
function extractErrorMessage(error: unknown, walletId?: string): string {
  ...
  const walletName = walletId ? WALLET_METADATA[walletId]?.name || walletId : 'wallet';

  // Handle empty error objects (xBull issue)
  if (error && typeof error === 'object' && Object.keys(error).length === 0) {
    console.warn(`[Wallet Adapter] Empty error object from ${walletName}:`, error);
  ...
  console.error(`[Wallet Adapter] Unknown error type from ${walletName}:`, error);
...
class StellarWalletAdapter {
  ...
  private getNetworkFromEnv(): WalletNetwork {
    ...
    return envNetwork === 'PUBLIC' ? WalletNetwork.PUBLIC : WalletNetwork.TESTNET;
  }
  ...
  async connect(): Promise<ConnectionResult> {
    ...
    if (isLocalhost()) {
      console.warn('[Wallet Adapter] Running on localhost. Albedo wallet may not work due to CORS restrictions. Use HTTPS or choose a different wallet.');
    }
      ...
      kit.openModal({
        onWalletSelected: async (option: ISupportedWallet) => {
          try {
            ...
            if (option.id === ALBEDO_ID && isLocalhost()) {
              ...
              try {
                ...
                const timeoutPromise = new Promise<never>((_, reject) =>
                  setTimeout(() => reject(new Error('Albedo connection timeout on localhost')), 5000)
                );
                ...
                const [{ address }, network] = await Promise.race([
                  ...
                  timeoutPromise
                ]) as [{ address: string }, { network: string; networkPassphrase: string }];
            ...
            if (walletInfo?.requiresEagerConnection && option.id !== ALBEDO_ID) {
            ...
            if (option.id === XBULL_ID) {
              ...
              while (retries < 3) {
                try {
                  ...
                  console.warn(`[Wallet Adapter] xBull getAddress attempt ${retries} failed, retrying...`);
                  await new Promise(resolve => setTimeout(resolve, 500));
                }
              }
            } else {
        ...
        onClosed: (error) => {
          if (error) {
            ...
            reject(new Error(errorMsg || 'Wallet selection cancelled or failed'));
          } else {
   ...
   */
  private async setWalletWithRetry(walletId: string, maxRetries = 3): Promise<void> {
    const kit = this.ensureKit();
    ...
    const actualMaxRetries = walletInfo?.requiresEagerConnection ? 1 : maxRetries;
    ...
    for (let attempt = 1; attempt <= actualMaxRetries; attempt++) {
      try {
        ...
        if (attempt < actualMaxRetries) {
          ...
          console.warn(`[Wallet Adapter] setWallet attempt ${attempt} failed for ${walletId}, retrying in ${delay}ms...`);
  ...
  async connectToWallet(walletId: string): Promise<ConnectionResult> {
    ...
    try {
      ...
      if (walletId === ALBEDO_ID && isLocalhost()) {
        console.warn('[Wallet Adapter] Albedo may not work on localhost due to CORS. Attempting connection...');
